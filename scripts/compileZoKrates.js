...
async function main() {
	...
	// read source
	[oneToOneMimcSource, fourToOneMimcSource, sendSource, withdrawSource] =
		await Promise.all([
	...
	const fileSystemResolver = (from, to) => {
		const location = path.resolve(
			path.dirname(path.resolve(circuitsDir + from)),
			to + ".zok"
		);
	...
	const oneToOneMimcArtifacts = zokratesProvider.compile(
		oneToOneMimcSource,
		options
	);
	const fourToOneMimcArtifacts = zokratesProvider.compile(
		fourToOneMimcSource,
		options
	);
	...
	const sendSolVerifier =
		spdx +
		zokratesProvider
			.exportSolidityVerifier(sendKeypair.vk, "v1")
			.replace("Verifier", "SendVerifier");
	const withdrawSolVerifier =
		spdx +
		zokratesProvider
			.exportSolidityVerifier(withdrawKeypair.vk, "v1")
			.replace("Verifier", "WithdrawVerifier");

	await Promise.all([
		// JSONs will get deleted by hardhat if placed on the same folder
		fsp.writeFile(
			artifactsDir + "1to1mimc/abi.json",
			oneToOneMimcArtifacts.abi
		),
		fsp.writeFile(
			artifactsDir + "4to1mimc/abi.json",
			fourToOneMimcArtifacts.abi
		),
		...
		fsp.writeFile(
			artifactsDir + "1to1mimc/program.bin",
			oneToOneMimcArtifacts.program
		),
		fsp.writeFile(
			artifactsDir + "4to1mimc/program.bin",
			fourToOneMimcArtifacts.program
		),
		fsp.writeFile(artifactsDir + "send/program.bin", sendArtifacts.program),
		fsp.writeFile(
			artifactsDir + "withdraw/program.bin",
			withdrawArtifacts.program
		),

		fsp.writeFile(artifactsDir + "send/proving_key.bin", sendKeypair.pk),
		fsp.writeFile(
			artifactsDir + "withdraw/proving_key.bin",
			withdrawKeypair.pk
		),

		fsp.writeFile(
			__dirname + "/../contracts/SendVerifier.sol",
			sendSolVerifier
		),
		fsp.writeFile(
			__dirname + "/../contracts/WithdrawVerifier.sol",
			withdrawSolVerifier
		),
	]).catch((error) => {
		console.log(error);
