...
describe("Vault (e2e test, should be around a minute)", function () {
	...
	before(async () => {
		...
		sendProvingKey = readProvingKey(
			__dirname + "/../build/circuits/send/proving_key.bin"
		);
		...
		withdrawProvingKey = readProvingKey(
			__dirname + "/../build/circuits/withdraw/proving_key.bin"
		);
	});
	...
	it("e2e test", async function () {
		...
		const secretId1 =
			"254080219567091772673909445246567392088208036796494585251815984474602972369";
		const secretId2 =
			"381081628386906533194738143309184053498881367127906410807449526152710014129";
		const nullifier1 =
			"272418632970930087013102078582984595767243027035861303396446195359448020486";
		const nullifier2 =
			"218278570709644582400354933544764600979629809596418196314625408049795099009";
		...
		let tx = await vault
			.connect(addr1)
			.deposit(toHex(deposit1.commitment), token.address, toHex(tokenId));
		await expect(tx)
			.to.emit(vault, "Deposit")
			.withArgs(toHex(deposit1.commitment), 0);
		...
		await expect(tx)
			...
			.withArgs(toHex(deposit1.nullifierHash));
		await expect(tx)
			.to.emit(vault, "Deposit")
			.withArgs(toHex(deposit2.commitment), 1);
		...
		await expect(tx)
			.to.emit(vault, "Withdrawal")
			.withArgs(toHex(deposit2.nullifierHash));
	});
});
...
async function createDeposit(secretId, nullifier, tokenContract, tokenId) {
	...
	const oneToOneMimc = (input) =>
		JSON.parse(
			zokratesProvider.computeWitness(oneToOneMimcArtifact, input).output
		)[0];
	const fourToOneMimc = (input) =>
		JSON.parse(
			zokratesProvider.computeWitness(fourToOneMimcArtifact, input).output
		)[0];

	deposit.publicId = oneToOneMimc([secretId]);
	deposit.nullifierHash = oneToOneMimc([nullifier]);
	// TODO: change order
	deposit.commitment = fourToOneMimc([
		deposit.publicId,
		nullifier,
		tokenContract,
		tokenId,
...
async function generateSendProof(
	vault,
	oldDeposit,
	newDeposit,
	artifact,
...
async function generateMerkleProof(contract, deposit) {
	...
	await expect(leafIndex).to.be.above(
		-1,
		"The deposit is not found in the tree"
	);
...
async function generateSnarkProof(artifacts, pk, input) {
	...
	const proofRaw = zokratesProvider.generateProof(
		artifacts.program,
		witness,
		pk
	);
